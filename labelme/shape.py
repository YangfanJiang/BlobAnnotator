        ...
        if self.points:
            ...
            if self.shape_type == "rectangle":
                ...
                if len(self.points) == 2:
                    ...
                    centreX = self.points[0].x()
                    centreY = self.points[0].y()
                    x =self.points[1].x()
                    y = self.points[1].y()
                    cxT = x - centreX
                    cyT = y - centreY
                    ...
                    for count in range(10):
                        newX = cxT * np.cos(count*2*np.pi/10) - cyT * np.sin(count*2*np.pi/10) + centreX
                        newY = cxT * np.sin(count*2*np.pi/10) + cyT * np.cos(count*2*np.pi/10) + centreY
                        p = QPointF()
                        p.setX(newX)
                        p.setY(newY)
                        points.append(p)
